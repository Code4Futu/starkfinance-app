...
interface Props {
  ...
  onChangeEventTypeFilter: (event: ActivityEvent) => void;
  collectionFilter: CollectionActivity[];
  onChangeCollectionFilter: (col: CollectionActivity) => void;
  chainFilter: CollectionChain[];
  onChangeChainFilter: (col: CollectionChain) => void;
  currencyFilter: CurrencyType[];
  onChangeCurrencyFilter: (col: CurrencyType) => void;
  handleClearAllFilter: () => void;
}

export const ModalActivityFilter = (props: Props) => {
  ...
  const items: CollapseProps["items"] = [
          ...
          {eventTypeList().map((item, idx) => (
            <div
              ...
              className={clsx(
                ...
                {
                  "bg-[#3C3D4D]":
                    props.eventType.find((e) => e === item.value) !== undefined,
                }
              )}
              onClick={() => props.onChangeEventTypeFilter(item.value)}
            >
    ...
    },
    {
      ...
      children: (
        <div className="flex flex-col items-start gap-3 self-stretch">
          ...
          />
          {collectionFilterList().map((item, idx) => (
            <div
              key={idx}
              className={clsx(
                ...
                {
                  "bg-[#2D313E]":
                    props.collectionFilter.find((e) => e === item.value) !==
                    undefined,
                }
              )}
            >
              <Checkbox
                onChange={() => props.onChangeCollectionFilter(item.value)}
                className="w-6 h-6"
              />
    ...
    },
    {
      ...
      children: (
        <div className="flex flex-col items-start gap-3 self-stretch">
          {collectionChainList().map((item, idx) => (
            <div
              key={idx}
              className={clsx(
                ...
                {
                  "bg-[#2D313E]":
                    props.chainFilter.find((e) => e === item.value) !==
                    undefined,
                }
              )}
            >
              <Checkbox
                onChange={() => props.onChangeChainFilter(item.value)}
                className="w-6 h-6"
              />
              <div className="relative w-9 h-9">
                <Image src="/svg/switch_network.svg" alt="" fill />
              </div>
              <div className="flex items-center gap-[10px] flex-1">
                <span className="text-base text-[#f1f1f1] leading-[19px]">
                  {item.label}
          ...
          {currencyList().map((item, idx) => (
            <div
              ...
              className={clsx(
                ...
                {
                  "bg-[#2D313E]":
                    props.currencyFilter.find((e) => e === item.value) !==
                    undefined,
                }
              )}
            >
              <Checkbox
                onChange={() => props.onChangeCurrencyFilter(item.value)}
                className="w-6 h-6"
              />
              <div className="flex items-center gap-[10px] flex-1">
                <span className="text-base text-[#f1f1f1] leading-[19px]">
