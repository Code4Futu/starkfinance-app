...
export const FilterProfileSwiper = ({
	...
	setProfileFilter: React.Dispatch<
		React.SetStateAction<ProfileFilter>
	>;
}) => {
						...
						(item, idx) => (
							<SwiperSlide
								...
								className={clsx(
									...
									item.value === "activity" &&
										"max-w-[105px]",
									item.value === profileFilter &&
										"!bg-[#F1F1F1]"
								)}
								onClick={() =>
									setProfileFilter(item.value)
								}
							>
								<span
									className={clsx(
