...
export default function Profile() {
  ...
  const [view, setView] = useState<string>("small");
  const [selectedItem, setSelectedItem] = useState<string[]>([]);
  const [openSelectFilter, setOpenSelectFilter] = useState<boolean>(false);
  const [selectFilter, setSelectFilter] = useState<SelectFilter>("newest");

  const handleOpenSelectFilter = (newOpen: boolean) => {
    setOpenSelectFilter(newOpen);
  };

  const handleSelectItem = (address: string) => {
    if (selectedItem.find((item) => item === address)) {
      setSelectedItem(selectedItem.filter((item) => item !== address));
      return;
    }
    ...
    <div>
      ...
      <div className="relative flex pt-6 flex-col gap-3 justify-between items-start mb-6 md:gap-6 md:mb-[500px] lg:mb-9 w-full">
              ...
              <Popover
                content={
                  <div className="flex p-3 flex-col items-center justify-center gap-[6px] rounded-3xl border-[1px] border-[#2D313E] bg-[#1A1C24]">
                    {selectFilterMockup().map((item, idx) => (
                      <div
                        key={idx}
                ...
                onOpenChange={handleOpenSelectFilter}
                arrow={false}
                placement="bottom"
              >
        ...
        <div className="w-full flex flex-col items-start pb-6 gap-6">
          ...
          />
          <OfferReceiveSection profileFilter={profileFilter} />
          <ListingSection profileFilter={profileFilter} />
          <ActivitySection profileFilter={profileFilter} />
        </div>
      </div>
      {isOpenFilter && (
        <ModalActivityFilter
